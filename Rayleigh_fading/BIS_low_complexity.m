...
% BIS_complexity:  Complexity

function [final_sum_rate, Switch_mat,BIS_complexity]=BIS_low_complexity(K,M,iters,pow_mat,H_noisy,H_nonoise,Ns,L,noise_cond)
    ...
    for pow_idx=1:length(pow_mat)
        ...
        for iter=1:iters
            ...
            [SINR_max,B_opt,complexity_iter]= BIS_local_BF (M,N,L,ini_switch,loc_matrix,All_values,H,P_r,N_P, SINR_max);
            complexity=complexity+complexity_iter;
            fix_loc_first=[];    
            ...
            while L*step1<M
                ...
                if length(ia)~=Ns
                    ...
                    ia=[ia;repmat(ia(end),Ns-length(ia),1)];
    ...
    % Average sum-rate
    BIS_complexity(1,pow_idx)=mean(complexity_mat);
