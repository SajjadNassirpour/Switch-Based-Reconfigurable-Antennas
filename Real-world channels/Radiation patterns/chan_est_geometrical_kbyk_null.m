...
function [chan_est,chan_est_nonoise,analogbeamforming_vec,analogbeamforming_vec_null,analogbeamforming_vec_null_multipath,bts_cords,refl_cords_all] = chan_est_geometrical_kbyk_null(user_locs, base_station_loc, chan_est_snr_db, grid_dim,  num_ants, support_user_index, enable_multipath, freq, rx_ant_sep_lambda, multipath_loss)
  % Default params 
  if nargin <= 6
  ...
  reflector_slope_intercepts = [[0,grid_dim];[90,grid_dim];[0,0];[90,0]];
  ...
  refl_cords_all = zeros([num_users,4,2]);
  ...
  for j = 1:1:num_users
      ...
      if(enable_multipath)
          for reflector_index=1:1:num_reflectors
            [reflected_distances, path_loss_facs, refl_cord] = get_reflected_dists(tiled_user_locs,curr_bts_cords,reflector_slope_intercepts(reflector_index,:));
            reflected_distances = reflected_distances.';
            avg_refl_loss = 1/(mean(path_loss_facs));
            refl_cords_all(j,reflector_index,:) = refl_cord;
...
function [reflected_dists, path_loss_facs,refl_cord] = get_reflected_dists(tiled_user_locs,dest_pts,reflector_slope_intercept)
