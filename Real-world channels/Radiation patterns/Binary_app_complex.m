...
% ffinal_sum_rate: Sum-rate
% Switch_mat :     The matrix of the optimal switch confgurations

function [ffinal_sum_rate, Switch_mat]=Binary_app_complex(K,M,iters,pow_mat,H_noisy,H_nonoise,mu,r,divide_rate,noise_cond)
    ...
    for pow_idx=1:length(pow_mat)
        ...
        for iter=1:iters
            ...
            Switch_mat(iter,pow_idx,BTS,:)=B_opt_LN_rand;
            ...
            % SINR
            score_LN_rand=-obj_func_diff_pow2(B_opt_LN_rand,H_correct,P_r,N_P);

            % Rate at each user
            rate_LN_rand(BTS,iter)=log2(1+score_LN_rand);

            end
        end
    final_sum_rate(1,pow_idx)=sum(mean(rate_LN_rand,2));

    end
    % Average sum-rate
    ffinal_sum_rate=final_sum_rate;  
end
