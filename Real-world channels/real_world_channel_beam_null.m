...
% kbyk_chans :          Noisy channels
% kbyk_chans_nonoise :  Noiseless channels
% kbyk_analogbeamformvecs :  Analog beamforming weighting vector 
% kbyk_analogbeamformvecs_nulls : Beamnulling + Analog beamforming weighting vector 


function [kbyk_chans,kbyk_chans_nonoise,kbyk_analogbeamformvecs,kbyk_analogbeamformvecs_nulls]=real_world_channel_beam_null(K,M,iters,est_SNR,user_distr)
...
while(chan_index<=num_chans)
    ...
    for j=1:1:num_bts
        curr_bts_cords = bts_cords(j,:);
        [kbyk_chans(chan_index,j,:,:), kbyk_chans_nonoise(chan_index,j,:,:), kbyk_analogbeamformvecs(chan_index,j,:,:), kbyk_analogbeamformvecs_nulls(chan_index,j,:,:),kbyk_analogbeamformvecs_nulls_multipath(chan_index,j,:,:)] =  chan_est_geometrical_kbyk_null(selected_user_cords, curr_bts_cords, chan_est_snr_db, grid_dim,  num_ants, j);
    end
